...
```
- INT 100で消費50%軽減
- 空腹ゲージと同期（フードレベルは19に制限、常に食事可能）
- **バニラの疲労（exhaustion）は無効** - 走る・ジャンプ・採掘などでマナは減らない
- マナは魔法詠唱でのみ消費される
...
```
食事回復:
  1. まずマナを回復（食べ物の回復量 + Cookingボーナス）
  2. マナがオーバーフローした場合、余剰分をHPに変換
     HP回復量 = オーバーフロー量 × 5（内部HP）
金リンゴ/再生ポーション: Regeneration効果中に内部HP += 回復量 × 10
魔法回復: 内部HP += Heal/Greater Heal の回復量 × 10
```
- **自然回復（SATIATED）は無効** - 食事によるオーバーフローでのみHPが回復
- Regeneration効果がないと、REGEN/MAGIC_REGENイベントはブロックされる
...
### マナシステム（空腹ゲージ連動）
- **マナ = 空腹ゲージ**（フードバーがマナを表す）
- 魔法を使うと空腹ゲージが減少
- **バニラの疲労（exhaustion）は完全無効** - 走る・ジャンプ・採掘などでは減らない
- フードレベルは19に制限（常に食事可能）

#### 食事による回復
1. 食べ物を食べるとまず**マナを回復**（食べ物の回復量 + Cookingボーナス）
2. マナが満タンを超える場合、**オーバーフロー分がHPに変換**される
   - 変換率: 1マナオーバーフロー = 5内部HP（0.25バニラハート）
3. HQ/EX品質の食べ物はHealth Boost効果も付与

#### HP回復手段
- **食事のオーバーフロー** - マナが満タン時に食事するとHPが回復
- **魔法** - Heal, Greater Heal
- **Regeneration効果** - 金リンゴ、再生のポーション、ビーコン
- ※バニラの自然回復（SATIATED）は**無効**
