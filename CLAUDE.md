...
本プラグインの世界観における伝説的な鍛冶師。ボスドロップやレア装備の製作者名として使用する。

| 名前 | 読み | 由来・設定 |
|-----|------|-----------|
| Zardoz | ザードス | 古代の魔法鍛冶師。魔力を宿した武具を鍛える |
| Volund | ヴェルンド | 北欧神話の鍛冶師に由来。堅牢な防具を得意とする |
| Mondain | モンデイン | UOの世界観に由来。神秘的な装飾品や特殊武器を作る |

- ボスドロップのEX品質アイテムには、これら3名のいずれかの名前がランダムで製作者名として付与される
- プレイヤーが作成したEX品質アイテムとの差別化として機能する（プレイヤー名 vs 伝説の鍛冶師名）
- 今後のボスコンテンツや特殊報酬でも統一的に使用する
...
#### 内部マナ（UOスタイル、空腹ゲージとは独立）
```
最大マナ = 20
実消費 = 基礎消費 × (1 - INT / 200)
```
- INT 100で消費50%軽減
- **空腹ゲージとは連動しない**（バニラの空腹システムは通常動作）
- マナは魔法詠唱でのみ消費される
- マナ回復方法:
  - **自然回復**: 5秒ごとに 0.5 × (1 + INT/100) マナ回復（INT 100で1.0/5秒）
  - **瞑想（Meditation）**: スニーク静止中に追加回復（Meditationスキル依存）
  - **金のリンゴ**: +5 MP
...
#### HP回復計算
```
食事: バニラ動作（空腹ゲージ回復 → 満腹時に自然回復）
金リンゴ/再生ポーション: Regeneration効果中に内部HP += 回復量 × 10
魔法回復: 内部HP += Heal/Greater Heal の回復量 × 10
```
- **バニラの自然回復**は有効（空腹ゲージ満タン時）
- HQ/EX品質の食事はHealth Boost効果も付与

#### マナ回復計算
```
自然回復: 5秒ごとに 0.5 × (1 + INT/100) マナ回復
瞑想: 0.5秒ごとに 0.1 × (1 + Meditation/100) マナ回復（スニーク静止時）
金のリンゴ: +5 MP
エンチャントされた金のリンゴ: +10 MP
```
...
| Wrestling | 素手 | 素手戦闘。ダメージ補正（0.5+スキル/100）、スタン確率（スキル/4 %、最大25%） | 素手でダメージを与えた時 |
...
###### 高品質食品のHealth Boost
HQ/EX品質の食べ物を食べると、一時的に最大HPが増加する。
| 品質 | 効果 | 持続時間 |
|------|------|----------|
| HQ | Health Boost I（+2ハート / +4HP） | 30秒 |
| EX | Health Boost I（+2ハート / +4HP） | 1分 |

※ 金リンゴ（Absorption + Regeneration）より控えめな効果
...
| Lumberjacking | 木材 | 伐採速度向上、連鎖伐採（スキルに応じて隣接する原木を自動伐採） | 原木を伐採した時 |
| Fishing | 魚、宝 | 自動釣り上げ（スキルに応じて自動で釣り上げ＆再キャスト） | 釣りで何かを釣った時 |
| Farming | 作物 | 自動植え直し、自動成長（スキルに応じて植えた作物が成長） | 成熟作物を収穫した時 |

#### 資源スキルの具体的効果
| スキル | 効果 | 計算式 |
|--------|------|--------|
| Mining | 採掘速度 | +（STR / 5 + スキル値 / 5）%（最大+40%） |
| Mining | レア鉱石ボーナスドロップ | スキル値 / 2 %（最大50%）で追加1個、発動時にパーティクル・サウンドで通知 |
| Mining | 経験値オーブ増加 | +（スキル値 / 5）%（最大+20%）※未実装 |
| Lumberjacking | 伐採速度 | +（スキル値 / 10）%（最大+10%） |
| Lumberjacking | 連鎖伐採 | 1 + floor(スキル値 / 10) ブロック（スキル100で無制限） |
| Fishing | 自動釣り上げ確率 | スキル値%（スキル20以上で発動、100で常時成功） |
| Farming | 自動植え直し | 収穫時に同じ種を持っていれば自動植え付け |
| Farming | 自動成長 | 植え付け時にスキルに応じて成長段階を進める（下記参照） |

#### 連鎖伐採システム（Lumberjacking）
原木を伐採すると、スキルに応じて隣接する同種の原木を自動的に伐採する。
...
##### 連鎖ルール
- **対象**: 原木のみ（葉は破壊しない）
- **範囲**: 上方向のみ（上、斜め上の9方向）※下方向には連鎖しない
- **耐久度**: 最初の1ブロック分のみ消費
- **スタミナ**: 消費なし
- **別種の木**: 連鎖しない（オーク→オークのみ）
- **斧の要件**: 不要（素手でも連鎖する）
- **ドロップ**: 各原木の位置にドロップし、自然落下する
- **処理方式**: 分散処理（数tickに分けて破壊、サーバー負荷軽減）

##### 連鎖の挙動
- **根本を切る**: 木全体が伐採される
- **真ん中を切る**: 切った位置から上だけが伐採される（下は残る）
- **上の方を切る**: その部分から上だけが伐採される

##### 連鎖アルゴリズム
```
1. プレイヤーが原木を破壊
2. 破壊した原木の種類を記録
3. 上方向（Y+1）および斜め上（9方向）のみを探索
4. 連鎖上限まで破壊（数tickに分散）
5. 各原木の位置にアイテムをドロップ（上から落下）
6. スキル上昇判定（1回のみ）
```

##### 分散処理の詳細
- 1tickあたりの最大破壊数: 5ブロック（config設定可能）
- 破壊順序: 上から下へ（逆順で破壊）
- 演出: 木が上から順に崩れ落ちるような自然な見た目

#### 自動農業システム（Farming）
成熟した作物を収穫すると、自動的に植え直しと成長が行われる。

##### 自動植え直し
- 成熟した作物を収穫した時、同じ種をインベントリに持っていれば自動で植え付け
- 種はインベントリから消費される
- 植え付け後、スキルに応じた自動成長が発動

##### 自動成長（骨粉効果）
植え付け時にスキルレベルに応じて作物の成長段階を進める（骨粉消費なし）。
...
| 作物 | 理由 |
|-----|------|
| メロン・カボチャ | 茎が残るため植え直し不要 |
| スイートベリー | 茂みが残るため植え直し不要 |
| 竹・サトウキビ・サボテン | 積み重ね式のため植え直し不要 |
| ココア | 木に付ける特殊作物 |
...
#### 自動釣りシステム（Fishing）
魚がかかると、スキルに応じた確率で自動的に釣り上げ、再キャストを行う。

##### 自動釣り上げ確率
| スキル値 | 確率 | 説明 |
|---------|------|------|
| 0-19 | なし | 手動のみ（バニラ動作） |
| 20-99 | スキル値% | 確率で自動釣り上げ |
...
2. 魚がかかる
3. 自動釣り上げ判定（スキル値%）
   ├─ 成功 → 自動で釣り上げ → 自動で再キャスト → 2へループ
   └─ 失敗 → 魚が逃げる → 手動で巻き上げるまで釣りは終わらない
4. GM（スキル100）は成功/失敗に関わらず常に自動で釣り上げ＆再キャスト
```

##### 特徴
- **スキル20-99**: 失敗時は手動で巻き上げが必要（釣りを再開するには手動リキャスト）
- **放置不可（低スキル）**: スキル20未満は完全手動
- **放置可能（GM）**: スキル100で無限ループ（釣り竿が壊れるまで）
- **停止条件**: 釣り竿の耐久度がなくなる、プレイヤーが手動で止める、移動やログアウト
- **失敗メッセージ**: 「集中が途切れた...魚に逃げられた！」

##### 釣り竿耐久度軽減
自動釣りでも手動釣りでも、スキルに応じて耐久度消費が軽減される。
```
軽減確率 = スキル値 × 0.9 %（スキル100で100%軽減）
```
...
- 成功率: スキル値%
- 解除条件: 攻撃、ブロック破壊、アイテム使用、ダメージを受ける、走る
...
| スキル名 | 効果 | 上昇条件 |
|---------|------|---------|
| Athletics | 落下ダメージ軽減（90%基本 + スキル×0.9%）、スキル100で完全免疫 | 落下ダメージを受けた時 |
| Swimming | 溺れダメージ軽減（90%基本 + スキル×0.9%）、空気消費軽減、スキル100で完全免疫 | 溺れダメージを受けた時 |
| Heat Resistance | 炎/溶岩ダメージ軽減（90%基本 + スキル×0.9%）、スキル100で完全免疫+消火 | 炎/溶岩ダメージを受けた時 |
| Cold Resistance | 凍結ダメージ軽減（90%基本 + スキル×0.9%）、スキル100で完全免疫+解凍 | 凍結ダメージを受けた時 |
| Endurance | 窒息/接触ダメージ軽減（90%基本 + スキル×0.9%）、スキル100で完全免疫 | 窒息/接触ダメージを受けた時 |

#### サバイバルスキル共通仕様

##### ダメージ軽減計算（内部HPシステム対応）
内部HPシステムはダメージを10倍で計算するため、サバイバルスキルで補正を適用:
```
基本軽減 = 0.1（90%軽減、バニラ相当に調整）
スキル軽減 = スキル値 × 0.9 %（最大90%）
最終ダメージ = 内部ダメージ × 基本軽減 × (1 - スキル軽減/100)
```
- スキル0: 内部ダメージの10%（≒バニラダメージ）
- スキル50: 内部ダメージの5.5%
- スキル100: 内部ダメージの1%（ほぼ無効）

##### GM（スキル100）完全免疫
スキル値が100に達すると、該当ダメージタイプに対して完全免疫:
- ダメージイベント自体がキャンセル（`event.isCancelled = true`）
- 画面揺れエフェクトも発生しない
- Heat Resistance: 炎上状態も即座に消火
- Cold Resistance: 凍結状態も即座にクリア

#### Athletics（運動能力）
- 効果: 落下ダメージを軽減
- 軽減率: 90%基本 + スキル×0.9%（スキル100で99%軽減）
...
#### Heat Resistance（耐熱）
- 効果:
  - 炎/溶岩ダメージを軽減（90%基本 + スキル×0.9%）
  - 炎上時間を短縮
  - スキル95以上: 炎上を消火する確率あり
  - スキル100: 完全免疫（画面揺れなし）+ 炎上状態を即座に消火
- 上昇条件: 炎/溶岩ダメージを受けた時
- 難易度: 溶岩60、火30、炎上20、マグマブロック25

#### Cold Resistance（耐寒）
- 効果:
  - 凍結ダメージを軽減（90%基本 + スキル×0.9%）
  - 凍結蓄積を軽減
  - スキル95以上: 凍結から脱出する確率あり
  - スキル100: 完全免疫（画面揺れなし）+ 凍結状態を即座にクリア
- 上昇条件: 凍結ダメージを受けた時
- 難易度: 40

#### Endurance（持久力）
- 効果:
  - 窒息ダメージを軽減（90%基本 + スキル×0.9%）
  - 接触ダメージを軽減（90%基本 + スキル×0.9%）
    - ※サボテン、ベリーの茂み等
  - スキル90以上: 窒息ダメージを完全回避する確率あり
  - スキル100: 完全免疫（画面揺れなし）
- 上昇条件: 窒息/接触ダメージを受けた時
- 難易度: 窒息50、接触15
...
### マナシステム（UOスタイル、空腹ゲージとは独立）
- マナは空腹ゲージとは**連動しない**（バニラの空腹システムは通常動作）
- 魔法を使うとマナが減少（スコアボードで表示）
- INT 100で魔法消費50%軽減

#### マナ回復手段
- **自然回復**: 5秒ごとに 0.5 × (1 + INT/100) マナ回復
  - INT 0: 0.5/5秒、INT 50: 0.75/5秒、INT 100: 1.0/5秒
- **瞑想（Meditation）**: スニーク静止中に追加回復
  - 0.5秒ごとに 0.1 × (1 + Meditation/100) マナ回復
  - Meditationスキル上昇判定あり
- **金のリンゴ**: +5 MP（HP回復効果に加えて）
- **エンチャントされた金のリンゴ**: +10 MP（HP回復効果に加えて）

#### HP回復手段
- **魔法** - Heal, Greater Heal
- **Regeneration効果** - 金リンゴ、再生のポーション、ビーコン
- **バニラの自然回復** - 空腹ゲージ満タン時の自然回復
- **食事** - 通常のバニラ動作（HQ/EX品質はHealth Boost効果も付与）

### 魔法レジスト
```
ダメージ軽減率 = Resist Spells × 0.7 (最大70%軽減)
...
Resist軽減 = 対象のResisting Spells × 0.7 / 100（最大70%軽減）
...
魔法系Mobの撃破、End City宝箱、トライアルチャンバー宝物庫からスクロールを入手できる。
...
#### End City宝箱
- 高サークル（C7〜C8）のスクロールが出現
- C7: 0.5%、C8: 0.2%

#### トライアルチャンバー宝物庫
- 全サークル（C1〜C8）のスクロールが出現
- 高サークルほど確率が低い
...
##### 合成（インベントリ内ドラッグ＆ドロップ）
Teleportスクロールとアメジストの欠片を合成してルーンを作成する。

| 項目 | 内容 |
|-----|------|
| 作成方法 | インベントリ内でTeleportスクロールとアメジストの欠片を重ねる |
| 必要アイテム | Teleportスクロール ×1、アメジストの欠片 ×1 |
| 必要スキル | なし |
| 成功率 | 100%（失敗なし） |
| 結果 | 空のルーン（Blank Rune）を取得 |

**作成手順：**
1. Teleportスクロールをカーソルで持つ
2. インベントリ内のアメジストの欠片をクリック
3. → 空のルーンが作成される（逆の操作でも可）

##### 素材の入手先
| 素材 | 入手方法 |
|-----|---------|
| Teleportスクロール | Mobドロップ（ウィッチ等）、End City宝箱、自作（`/scribe teleport`） |
| アメジストの欠片 | アメジストクラスターを採掘（アメジストジオード内） |

#### 空のルーンの使い方
1. 空のルーンをメインハンドに持つ
2. `/cast mark` を実行（Mark魔法で場所を記録）
3. 地点をクリック → 記録済みルーンになる
4. 記録済みルーンを使って Recall / Gate Travel が可能
...
複数のルーンを保存できる魔法の本。チェストのようにルーンを出し入れでき、GUIからRecallとGate Travelを直接実行できる。
...
#### GUI設計（18スロット = 2行）
```
Row 1: [Rune1][Rune2][Rune3][Rune4][Rune5][Rune6][Rune7][Rune8][空き]
Row 2: [Rune9][Rune10][Rune11][Rune12][Rune13][Rune14][Rune15][Rune16][空き]
```

- チェストのようにルーンアイテムを自由に出し入れ可能
- **登録可能**: マーク済みルーン（位置情報が記録されたアメジストの欠片）のみ
- **登録不可**: マークされていないルーン、その他のアイテム

#### 操作方法
| 操作 | 効果 |
|-----|------|
| 左クリック | ルーンを取り出す（通常のチェスト動作） |
| 右クリック | Recall詠唱開始（GUI閉じる→詠唱→テレポート） |
| Shift+右クリック | Gate Travel詠唱開始（GUI閉じる→詠唱→ゲート生成） |
| ドラッグ＆ドロップ | ルーンの移動・登録（通常のチェスト動作） |
| Shift+左クリック | ルーンを素早く移動（通常のチェスト動作） |

※ 右クリック系以外の操作は通常のチェストと同じ動作
...
Wrestling補正 = 0.5 + (Wrestling / 100)   // 0.5〜1.5、素手攻撃時のみ適用
...
#### 素手戦闘（Wrestling）
素手攻撃はWrestlingスキルによりダメージとスタン効果が強化される。

##### 基礎ダメージ
- 素手の基礎ダメージ: 3
- Wrestling 100の場合: 3 × 1.5 = 4.5（木の剣〜鉄の剣相当）

##### スタン効果
| Wrestling | ダメージ補正 | スタン確率 |
|-----------|------------|----------|
| 0 | ×0.5 | 0% |
| 50 | ×1.0 | 12.5% |
| 100 | ×1.5 | 25% |

- **スタン持続時間**: 1秒（Slowness 255 + Jump Boost -128）
- **スタンクールダウン**: 同一対象への連続スタンは5秒間不可
- **対象**: Mob/プレイヤー問わず適用
...
### 基本仕様
UOスタイルの防具システムを採用。各防具にはAR（Armor Rating）、STR必要値、DEX必要値、DEXペナルティが設定される。
...
|-----|---:|--------:|--------:|------------:|
| ネザライトヘルメット | 8 | 80 | 35 | 5 |
| ネザライトチェストプレート | 18 | 95 | 50 | 12 |
| ネザライトレギンス | 14 | 85 | 45 | 8 |
| ネザライトブーツ | 8 | 70 | 35 | 4 |
| ダイヤヘルメット | 7 | 65 | 30 | 4 |
...
### DEX必要値
防具を装備するために必要な最低DEX値。DEXが不足していると装備不可（UOスタイル）。
- 装備試行時にDEXをチェック
- DEXが低下した場合、装備解除される
- メッセージ: 「DEXが足りません！(必要: {required}, 現在: {current})」
- STR/INT特化ビルドはプレート防具を装備できず、チェーン以下を使用する必要がある

#### フルセット装備時のDEX必要値
| 防具セット | DEX必要値（最大パーツ） | 備考 |
|-----------|:--------------------:|------|
| フルネザライト | 50 | チェストが最高要求 |
| フルダイヤ | 45 | チェストが最高要求 |
| フル鉄 | 35 | チェストが最高要求 |
| フルチェーン | 25 | チェストが最高要求 |
| フルレザー/金 | 0 | 制限なし |
